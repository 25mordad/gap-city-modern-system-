...
if ($_GET['step'] == "title" ){
	...
	$ReferenceMango =  explode(" ",$Reference[0]);
 	...
 	foreach ($html->find('span[class=strikethrough]') as $prices) {
		$PricesMain[] = $prices->plaintext;
	}
	
	$realPrice = trim($PricesMain[0])+1;
	
 	foreach ($html->find('span[class=ficha_precio_venta_entero]') as $prices) {
		$PricesBuy[] = $prices->plaintext;
	}
	
	$buyPrice = trim($PricesBuy[0])+1;
...
foreach ($html->find('input[class=inputOcultoColor]') as $sizes) {
	...
	foreach ($AllsizesArr as $row){
		if (strpos($row[2], 'disponible') != true){
...
$scrptTxt = "";
	...
	if ($_GET['upd'] == "true")
	{
		...
		header("location: /gadmin/shop/importFromMangooutlet/$id?step=price");
	}
